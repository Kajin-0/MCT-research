...
def novik_bulk_reference(
    ...
    return np.asarray(
        [
            ...
            [p * kp / rt6, p * np.sqrt(2.0 / 3.0) * kz, np.conjugate(r), 0, u - v, np.conjugate(s_plus), -np.sqrt(3.0 / 2.0) * s_plus, -rt2 * v],
            [0, p * kp / rt2, 0, np.conjugate(r), s_plus, u + v, rt2 * np.conjugate(r), s_plus / rt2],
            [-p * kz / rt3, -p * km / rt3, np.conjugate(s_minus) / rt2, rt2 * v, -np.sqrt(3.0 / 2.0) * np.conjugate(s_plus), rt2 * r, u - params.delta, 0],
