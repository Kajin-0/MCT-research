...
def analyze(path: str | Path) -> dict[str, Any]:
    ...
    maximum_model_equivalent_composition_offset = max(
        abs(payload["offset_from_nominal_x"]) for payload in inferred.values()
    )
    return {
        ...
        "cross_model_diagnostic": {
            "maximum_model_equivalent_composition_offset": (
                maximum_model_equivalent_composition_offset
            ),
            "composition_precision_needed_for_strict_ranking": (
                "independent homogeneous specimen-level uncertainty substantially "
                "smaller than the model-equivalent composition offsets"
            ),
        },
        "decision": {
            "primary_constraint_admitted": True,
            "central_source_recovery_status": "conditional",
            ...
            "independent_cross_laboratory_validation": False,
            "global_model_promotion_authorized": False,
            "coefficient_refit_authorized": False,
            "new_static_refit_authorized": False,
