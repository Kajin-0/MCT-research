"""Inference and HgCdTe propagation for scale-dependent spatial disorder.

This module extends the exact Gaussian covariance/kernel core without replacing
it.  It adds:

- the explicit one-scale non-identifiability family;
- the exact two-scale inverse in arbitrary spatial dimension;
- logarithmic conditioning diagnostics for experimental scale selection;
- exact one-dimensional top-hat formulas for Gaussian and exponential
  covariance;
- second-order propagation of probe-averaged composition through a declared
  HgCdTe signed-gap law;
...
def single_scale_point_variance_family(
    ...
    *,
    ...
    For a Gaussian covariance and Gaussian probe,

    .. math::

       V_a=\sigma_x^2\left(1+2a^2/\ell^2\right)^{-D/2}.
    ...
    One probe scale therefore cannot identify point variance and correlation
    length separately.
    """
...
def two_scale_log_parameter_condition_number(
    ...
    *,
    ...
    The condition number diverges for equal scales and becomes large when both
    probes occupy the same small-scale or large-scale asymptotic regime.
    """
...
def recover_isotropic_gaussian_two_scales(
    ...
    *,
    ...
    The function rejects data that cannot correspond to a positive isotropic
    Gaussian covariance under the declared dimension.
    """
...
def top_hat_gaussian_effective_variance_1d(
    ...
    .. math::
       ...
       V_{L,G}=\frac{2\sigma_x^2}{L^2}\left[
       L\ell\sqrt{\frac\pi2}\operatorname{erf}
       \left(\frac{L}{\sqrt2\ell}\right)
...
def top_hat_exponential_effective_variance_1d(
    ...
    .. math::
       ...
       \left[L\ell-\ell^2\left(1-e^{-L/\ell}\right)\right].
    """
...
def probe_averaged_gap_moments(
    ...
    *,
    ...
    A quadratic expansion around :math:`\bar x` gives
    ...
    The variance expression is exact for a quadratic composition dependence and
    Gaussian probe-averaged composition.
    """
