...
def schmit_stelzer_1969_gap_ev(
    x: ArrayLike, temperature_k: ArrayLike
) -> GapValue:
    """Return the Schmit-Stelzer detector-cutoff equation in eV.

    The published expression is

    ``Eg = 1.59x - 0.25 + 5.233e-4*T*(1 - 2.08x) + 0.327x^3``.

    It was fitted to half-peak photoconductive and photovoltaic detector cutoffs
    after specimen compositions were adjusted within their reported metrology
    limits. It is retained as an observation-class comparator, not as a
    method-free latent-gap equation.
    """
