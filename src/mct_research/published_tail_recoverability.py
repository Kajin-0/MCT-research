...
def _dimensionless_log_shape(
    ...
    *,
    ...
    moment = gaussian_gap_convolved_power_absorption(
        standardized_energy,
        mean_gap_ev=0.0,
        gap_sigma_ev=1.0,
        exponent=exponent,
...
def _window_coordinates(
    ...
    *,
    ...
    dense_count = max(8 * sample_count, 2049)
...
def gaussian_power_tail_window_straightness(
    ...
    *,
    ...
    ``upper_standardized_energy`` locates the upper absorption bound relative to
    the latent mean gap.  It is restricted to the controlled subgap domain
    ``z <= 0``.  The lower endpoint is selected so that the Gaussian-power shape
    spans exactly ``dynamic_range_decades``.

    Vertical residuals fit ``log10(alpha)`` as an affine function of ``z`` using
    uniform standardized-energy sampling.  Horizontal residual fractions fit
    ``z`` as an affine function of ``log10(alpha)`` using uniform log-absorption
    sampling; these fractions map directly onto the horizontal span of a plotted
    trace.
    """
    ...
    vertical_coefficients = np.linalg.lstsq(
        vertical_design, log_energy, rcond=None
    )[0]
    ...
    horizontal_coefficients = np.linalg.lstsq(
        horizontal_design, z_log, rcond=None
    )[0]
    ...
    return GaussianTailWindowStraightness(
        ...
        rms_vertical_residual_decades=float(
            np.sqrt(np.mean(vertical_residual**2))
        ),
        maximum_horizontal_residual_fraction=float(
...
def gaussian_power_tail_pixel_recoverability(
    ...
    *,
    ...
    """Map the curved model trace into plot pixels and fit a total-least-squares line."""

    horizontal_span = _finite_positive(
        "trace_horizontal_span_px", trace_horizontal_span_px
    )
...
def critical_upper_standardized_energy(
    ...
    *,
    ...
    """Return the least restrictive upper ``z`` that reaches a pixel threshold.

    The returned value is the critical upper standardized energy.  Curvature is
    resolvable only for windows with ``z_upper`` greater than or equal to this
    value.  ``None`` means the threshold is not reached anywhere in the declared
    subgap domain ``z_upper <= 0``.
    """

    sigma_multiple = _finite_positive("significance", significance)
    target = sigma_multiple * _finite_positive(
        "center_uncertainty_px", center_uncertainty_px
    )
    z_limit = _finite_positive("standard_deviation_limit", standard_deviation_limit)
...
def finkman_modified_urbach_trace_span_ev(
    ...
    Only the energy difference is used, so composition-dependent intercept and
    bowing terms cancel.  This function reproduces the empirical modified-Urbach
    slope used to draw the 1979/1984 source lines; it is not a Gaussian-disorder
    parameterization.
    """
...
def source_conditioned_finkman_trace(
    *,
    ...
    uncertainty = _finite_positive(
        "marker_center_uncertainty_px", marker_center_uncertainty_px
    )
    ...
    trace_dynamic_range = log10(
        absorption_max_cm_inverse / absorption_min_cm_inverse
    )
