...
The theorem label identifies an exact statement under declared assumptions. It does **not** assert unprecedented general mathematics. Novelty status is controlled separately by `data/validation/flagship_novelty_claim_matrix.json`.
...
| C09 | For an exponential tail, detector cutoff shifts exactly as `-W ln(d2/d1)`. | Exact forward-model theorem; underlying Beer-Lambert and thickness physics are prior art | `detector_cutoff.py`; derivation 011; Chang 2006/2007 | Complete | “Combining the Chang tail with the single-pass response gives the exact logarithmic shift.” | “We discover that detector cutoff depends on thickness.” |
...
| C11 | Any number of tail-only Chang cutoff observations has Jacobian rank at most two for `(Eg,W,ln A,ln b)`. | Exact application-specific structural theorem | derivation 011; `chang_2006_cutoff_identifiability.json` | Complete | “We derive a model-specific rank-two bound for tail-only HgCdTe cutoff datasets.” | “This is a new universal theorem for all detector cutoffs.” |
...
| C16 | The unmarked unified single-state spectrum depends on five nominal parameters only through translated mean gap, gap width, and `A*d`. | Exact application-specific structural theorem | `unified_spectrum.py`; derivation 013 | Complete | “Under the declared Gaussian/power-law/single-pass model, we derive the explicit identifiable combinations.” | “We introduce identifiable combinations or the Beer-Lambert product degeneracy.” |
| C17 | The unmarked dense-spectrum Jacobian has structural rank at most three. | Exact application-specific theorem plus numerical verification | `unified_spectrum_structural_rank.json` | Complete | “We derive a model-specific rank bound; no increase in SNR or point count removes its exact invariances.” | “We prove unprecedented general structural-identifiability mathematics.” |
| C18 | Two physically different parameter sets produce 281-point spectra equal to within `2.22e-16`. | Exact-invariance counterexample plus numerical verification | `unified_spectrum_structural_rank.json` | Complete | “The two parameterizations preserve the exact identifiable combinations.” | Presenting either parameter set as a real specimen or generalizing to all spectra. |
| C19 | An absolutely calibrated nontranslational carrier marker raises rank to four but leaves one combined null vector `(Delta,-Delta,0,-1,+1)`. | Exact marked-model invariance plus numerical verification | `unified_spectrum_structural_rank.json`; derivation 013 | Complete | “A controlled carrier-sensitive marker adds one independent direction while one external scale remains necessary.” | Calling the generic marker the Dingrong free-carrier law. |
| C20 | Independent carrier-state and effective-thickness information are necessary for latent-gap recovery under the base model. | Corollary of exact theorem | theorem index; derivation 013 | Complete | State as a model-conditioned measurement-design requirement consistent with prior optical inversion literature. | Claiming these measurements are sufficient for every real HgCdTe spectrum. |
...
| C23 | The physical Dingrong carrier physics breaks the unified gap/carrier degeneracy. | External/model validation | Dingrong 1985, DOI `10.1016/0038-1098(85)90315-1`; `dingrong1985_table1_reproduction.json` | **Partial** | “The source finite-temperature density integral exposes a printed-parameter inconsistency; the full below-gap carrier marker remains open.” | Presenting the diagnostic `E^-2` marker or the row-implied `P` as established universal physics. |

## Audited novelty hierarchy

### Established ingredients — do not claim novelty

1. structural identifiability as a concept;
2. output-preserving parameter symmetries;
3. identifiable parameter combinations;
4. the Beer-Lambert optical-depth product;
5. effective-thickness-dependent cutoff physics;
6. Gaussian gap distributions producing apparent exponential tails.

### Application-specific analytical contributions

1. the explicit HgCdTe combinations `Eg0+Delta`, `sigma_G`, and `A*d`;
2. the resulting rank-three bound for the declared distributed spectrum;
3. the tail-only Chang rank-two bound;
4. the marked-model combined null vector;
5. transition-root censoring in the declared near-inversion model.

These are exact under the declared assumptions, but are not described as new general mathematics.

### Strong project-specific quantitative findings

1. the machine-precision five-parameter spectral counterexample;
2. `60.1%` fit-window dependence of apparent tail energy;
3. mixed-branch condition number `199.81` in the declared design;
4. `11.297 meV` RMS mismatch between Dingrong's printed Eq. 2/printed `P` and Table 1.

## Publication framing

The manuscript is an **HgCdTe-specific semiconductor optical-metrology and inverse-problem methods paper**. It is not a general structural-identifiability paper, a universal gap-equation paper, or a complete microscopic absorption theory.

## Submission boundary

Claims C01–C20 support an analytical and reproducible methods paper. C21 has a qualified real-specimen source-table test but remains incomplete because native calibrated spectra and covariance are unavailable. C22 is optional. C23 is only partially supported until the complete physical carrier-dependent spectral branch is reconstructed.
