...
A hypothesis-driven, reproducible research portfolio for the electronic structure, transport physics, and measurable observables of $\mathrm{Hg}_{1-x}\mathrm{Cd}_x\mathrm{Te}$ (HgCdTe/MCT).
...
Define and test a self-consistent one-dimensional drift-diffusion-Poisson-Langevin theory for finite HgCdTe photoconductors. The program targets terminal-current noise, contact and diffusion modes, space-charge correlations, fluctuation-dissipation verification, and controlled failure criteria for the lumped single-Lorentzian GR model. Only Phase 1 literature and model definition are currently authorized.

State: `research/programs/stochastic_transport_noise/state.md`

## Shared scientific perspective

Across the band-edge programs, a reported HgCdTe edge is treated as an observable generated by a specimen and measurement process, not automatically as

$$
E_g(\bar x,T).
$$

The corresponding forward structure is
...
The stochastic-transport program applies the same observable-first discipline to detector noise:

```text
material, geometry, contacts, and terminal circuit
-> nonlinear carrier and electrostatic state
-> linearized transport and recombination dynamics
-> physically derived stochastic source covariance
-> conserved terminal-current or terminal-voltage spectrum
```

Different programs address different links in these structures and may produce separate completed works.
...
- program-level specifications for stochastic transport, parameter provenance, and literature auditing, with no stochastic solver yet claimed as validated.
